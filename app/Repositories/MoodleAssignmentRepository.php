...
class MoodleAssignmentRepository implements MoodleAssignmentInterface
{
    ...
    public function getAssignmentDetail($id, $userId)
    {
        ...
        if ($submission) {
            // Không chỉ check 'submitted', vẫn nên lấy file nếu status là 'new'
        ...
        $context = DB::table('mdl_context')
            ...
            ->where('instanceid', $assignment->cmid) // cmid là course_modules.id
            ->first();
        ...
        if ($context) {
            $teacherFiles = DB::table('mdl_files')
                ...
                ->where('itemid', 0) // intro files luôn có itemid = 0
